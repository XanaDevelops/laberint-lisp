...
(defun algoritmeDFS () 
  (let* 
    ...
    (completarLaberint laberint pos)
  )
)
...
(defun randomizedPrim () 
  ...
  (let* 
    ((laberint (create-matrix FILES COLUMNES paret)) 
      ...
      (laberint1 (setMatrixValue laberint cami pos))
      ; afegir els parets de la casella
      (paretsL (getLlistaAdjacentsX pos laberint1 paret))
      (newLaberint (recursivePrim paretsL laberint1))
    )
    (completarLaberint newLaberint)
  )
)



(defun completarLaberint (laberint &optional (posEntr)) 
  (let* 
    ((laberintAmbParetsExternes (setEdgesParet laberint)) 
      ...
      (posEntrada 
        (cond ((null posEntr) (getPrimerODarrer caminsAccessibles)) (t posEntr))
      )
    ...
    (format t " posEntrada ~a%" posEntrada)
    (writeToFile laberintComplet laberintOutputFile)
  )
)
...
;--------------VERSIÓ INICIAL DE RANDOM DIVISION--- GENERA LABERINTS NO NECESSARIAMENT CONNEXOS
  ; (defun-tco 
  ;   divide-recursiu
